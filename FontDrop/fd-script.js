...
(function(win, doc){
	...
	function changeFont(e) {
		...
		if( getSel !== "" ) {
			...
			var _this = this,
				current = _this[_this.selectedIndex],
				value = current && current.value ? current.value.split(" - ") : "";

			if(current && value[0].indexOf("GWF") !== -1) {
				currentFont = value[1];
				load("gwf", value[1], true);
			}
	...
	var gwf = function(data) {
		var fontFamilies = [], opts, font;
		selectorVal.className = "loader";
		each(data.items, function(obj, i){
			font = obj.family;
			appendOptionElem(font, "GWF - " + font);
	...
	function loadJSONP(url) {
	    with(document) body.appendChild(createElement(d = "script")).src = url;
	}
